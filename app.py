...
def admin_v54():
    ...
    try:
        with db() as conn:
            ...
            """).fetchall()
            stats = {
                "topics": conn.execute("SELECT COUNT(*) FROM teme").fetchone()[0],
                "opinions": conn.execute("SELECT COUNT(*) FROM svjetionik_misljenja").fetchone()[0],
                "replies": conn.execute("SELECT COUNT(*) FROM svjetionik_odgovori").fetchone()[0],
                "predictions": conn.execute("SELECT COUNT(*) FROM svjetionik_predvidjanja").fetchone()[0],
                "open_predictions": conn.execute(
                    "SELECT COUNT(*) FROM svjetionik_predvidjanja WHERE status='otvoreno'"
                ).fetchone()[0],
                "participants": conn.execute("SELECT COUNT(*) FROM korisnici").fetchone()[0],
                "analyses": conn.execute("SELECT COUNT(*) FROM svjetionik_analize").fetchone()[0],
            }
        return render_template("admin_v54.html", topics=topics, stats=stats)
    except Exception:
        app.logger.exception("Admin V5.4 nije uspio")
