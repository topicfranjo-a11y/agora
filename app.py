...
# ==============================================================================
# PRIPREMA STIMULATIVNIH TEKSTOVA ZA TEME (Upiši svoje tekstove ovdje)
# ==============================================================================
stimulacije = dict()
...
# Defaultni tekst ako tema nema definiranu stimulaciju
...
# KLJUČNO: Pratimo je li korisnik potvrdio razumijevanje trenutne teme
...
with col1:
    ...
    # Dohvaćamo tekst stimulacije za odabranu temu
    tekst_stimulacije = stimulacije.get(izabrana_tema, provokacija_default)
    
    # Provjeravamo je li korisnik već kliknuo "potvrdio" za OVE SELEKTIRANU TEMU
    je_potvrđeno = st.session_state.potvrđene_teme.get(izabrana_tema, False)
    
    if not je_potvrđeno:
        # PRIKAZ PROZORA ZA STIMULACIJU (Dok korisnik ne klikne gumb)
        with st.container(border=True):
            st.markdown("### 🏛️ Stimulacija za um (Provokacija)")
            st.warning(tekst_stimulacije)
            
            # Gumb za otključavanje unosa misli
            if st.button("🤝 Potvrđujem da sam shvatio poantu", use_container_width=True):
                st.session_state.potvrđene_teme[izabrana_tema] = True
                st.rerun()
    else:
        # PRIKAZ UNOSA TEKSTA (Pojavljuje se tek nakon kliknute potvrde)
        st.success("🔓 Pristup otključan: Protokol razumijevanja uspješno izvršen.")
        ...
        # Mala tipka za povratak na tekst provokacije ako korisnik želi ponovno pročitati
        if st.button("↩️ Ponovno pročitaj stimulaciju", help="Vraća prozor s provokacijom"):
